...
;(async () => {
  try {
    ...
    const contractERC20 = await deploy('ERC20', ["pencil qbx", "PEN", ownerAddress])
    console.log(`✅ Contract deployed at: ${contractERC20.address}`)
    ...
    console.log(`✅ Contract deployed at: ${contractFaucet.address}`)

    //3. transfer
    const amount = ethers.utils.parseUnits("10", 18); // 10 PEN (decimals = 18)
